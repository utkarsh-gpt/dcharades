import { HeadToHeadCard, MovieCard } from '../shared/types';

export const HEAD_TO_HEAD_CARDS: HeadToHeadCard[] = [
  // Sports Movies
  {
    id: 'sports-1',
...
export const BLOCKBUSTER_MOVIE_CARDS: MovieCard[] = [
  // Action Movies
...
export function getRandomHeadToHeadCard(): HeadToHeadCard {
  return HEAD_TO_HEAD_CARDS[Math.floor(Math.random() * HEAD_TO_HEAD_CARDS.length)];
}

export function getRandomMovieCards(count: number = 6): MovieCard[] {
  const shuffled = [...BLOCKBUSTER_MOVIE_CARDS].sort(() => 0.5 - Math.random());
  return shuffled.slice(0, count);
}

export function getMovieCardsByGenre(genre: string): MovieCard[] {
  return BLOCKBUSTER_MOVIE_CARDS.filter(card => card.genre === genre);
}

export function validateMovie(movieTitle: string, category: string): boolean {
...
function getCategoryMovies(category: string): string[] {
